...
def main():
    ...
    # Ensure schema consistency for all segments, including empty/uncovered ones.
    for seg in segments:
        seg["chunk_id"] = seg.get("chunk_id")
        seg["topic_id"] = seg.get("topic_id", -1)
        seg["topic_words"] = seg.get("topic_words", [])
        seg["entities"] = seg.get("entities", [])
    ...
    covered_non_empty = sum(len(group) for group in grouped_segments)
    remaining = len(non_empty_segments) - covered_non_empty
    if remaining > 0:
